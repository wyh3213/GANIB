...
def information_bottleneck_loss(vec_mean, vec_cov, beta=1e-3):
	"""信息瓶颈损失函数 - 简化版本"""
	# 更稳定的KL散度计算
	kl_divergence = 0.5 * torch.sum(vec_mean**2 + vec_cov**2 - torch.log(vec_cov**2 + 1e-8) - 1, dim=1)
	return beta * kl_divergence.mean()
...
parser.add_argument('--warmup_updates', type=int, default=400,
					help='warmup steps')
parser.add_argument('--peak_lr', type=float, default=0.002,
...
parser.add_argument('--n_heads', type=int, default=8,
					help='number of attention heads in model.')
...
# data read
Adj, Dis_adj, Meta_adj, feature, random_index, k_folds,Dis_feature, Meta_feature,Dis_simi,Meta_simi = load_data(args.seed, args.node_input)
...
for k in range(k_folds):  # Five-fold cross validation
	...
	best_recall = 0
	best_precision = 0
	best_model_state = None  # 新增：用于保存最佳模型的状态字典
	for epoch in range(args.epochs):  # 500 epochs per fold
		...
		# 接收信息瓶颈参数
		# output, vec_mean, vec_cov = model(processed_features, dis_data, meta_data).to(device)
		output, vec_mean, vec_cov = model(processed_features, dis_data, meta_data)
		output = output.to(device)
		vec_mean = vec_mean.to(device)
		vec_cov = vec_cov.to(device)
		...
		# loss_train = criterion(trian_scores, trian_labels).to(device)  +  information_bottleneck_loss(vec_mean, vec_cov, beta=1e-2)		# 信息瓶颈损失项
...
# 保存 average_5 到文本文件
file = open(f"../result/GANIB.txt", mode='a+')
